...
export const InputCode = observer(({ name }: InputCodeProps) => {
  ...
  const processPaste = useCallback(async (paste: string) => {
  ...
  const handleChange = async (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    setCode((prevCode) =>
      prevCode.map((digit, idx) => (idx === index ? value : digit))
    );
  ...
  const onInputPaste = useCallback(async (event: React.ClipboardEvent<HTMLInputElement>) => {
